...
class JobcardModel extends MY_Model {
	...
	public function getJobCardById($id) {
		$this->db->select('jc.*,jci.id AS image_id,jci.image,ro.id AS repair_order_id,ro.parts_name,ro.customer_request,ro.sa_remarks,ro.qty,ro.price_labour,ro.price_parts,ro.price_total,c.registration_no,c.color,cb.brand_name,cm.model_name,u.phone,u.email,u.name,se.loaner_vehicle,se.address,ei.id AS enquiry_image_id,ei.image AS enquiry_image'
		);
